...
if [ ${dataset} == "cuhk" ]; then let epoch=30*${NGPU}-1; elif [ ${dataset} == prw ]; then let epoch=30*${NGPU}-1; else let epoch=10*${NGPU}-1; fi;

# Train
python -m torch.distributed.launch --nproc_per_node=${NGPU} --use_env --master_port=55555 train.py --cfg configs/${dataset}.yaml --world-size ${NGPU} --resume --ckpt ../epoch_0.pth
